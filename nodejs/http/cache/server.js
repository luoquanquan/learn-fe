...
const checkEtag = (filePath, lastEtag) =>
...
http
  .createServer((request, response) => {
    ...
    const {
      headers: { ['if-modified-since']: lastModifiedTime, ['if-none-match']: lastEtag }
    } = request
    ...
    fs.stat(filePath, async (err, st) => {
      ...
      if (st.isFile()) {
        ...
        const { hited: hitLastModified, lastModified } = await checkLastModified(
          st,
          lastModifiedTime
        )
        if (hitLastModified) {
