...
经排查发现，移动端为了节约流量不会预加载视频资源(即使设置了 preload 属性)只有用户点击了播放之后视频才会开始加载。然而提取的音频都是内存里现成的资源触发播放就会跑起来。好在根据『背景视频无法自动播放』步骤中得知，静音的视频可以启动自动播放。于是只需要页面加载后自动播放视频一帧即可：
...
搞不定了，现在开始刷算法抱佛脚好像有点来不及。于是扯着脸找算法大佬 review 代码求指导。大佬一句话直接让我再跪：你直接改成 `start / 50` 在取整就可以知道当前弹幕的开始区块了。结束区块道理一致
...
经过旷日持久的搜索，我并没有发现解决办法。`AudioContext` 播放的音频属于背景音乐静音模式必然没有声音。好在经过了一番游说产品经理再次妥协。
