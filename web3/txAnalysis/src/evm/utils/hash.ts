...
const functionSelector = (abiItem) =>
  toFunctionSelector(formatAbiItem(abiItem));
export const erc20Info = {
  functionSelector: {
    approve: functionSelector(
      erc20Abi.find((x) => x.type === "function" && x.name === "approve"),
    ),
  ...
  eventSelector: {
    // 授权 Event
    Approval: eventSelector(
      erc20Abi.find((x) => x.type === "event" && x.name === "Approval")!,
    ),
    // 授权 Event
    Transfer: eventSelector(
      erc20Abi.find((x) => x.type === "event" && x.name === "Transfer")!,
    ),
  },
  functionNameMap: {
    approve: "approve",
