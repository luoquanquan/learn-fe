...
const transferFrom = (trace) => {
  try {
    ...
    const selectorMatched =
      selector === erc20Info.functionSelector.transferFrom;
    ...
    if (
      decodedByInputData.functionName !== erc20Info.functionNameMap.transferFrom
    )
      return null;
    const [
      decodedByInputDataFrom,
      decodedByInputDataTo,
      decodedByInputDataValue,
