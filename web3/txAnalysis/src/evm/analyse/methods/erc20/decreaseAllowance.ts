...
const decreaseAllowance = (trace) => {
  try {
    const { input = '', from, to: tokenAddress, logs = [], gasUsed } = trace
    // 第一步: selector 校验
    const selector = input.slice(0, 10).toLowerCase()
    const selectorMatched = selector === erc20Info.functionSelector.decreaseAllowance

    if (!selectorMatched) return null

    // 第二步: 基于 inputData 解码
    const decodedByInputData = decodeFunctionData({
      abi: [erc20ExtAbiMap.decreaseAllowance],
      data: input
    })
    if (decodedByInputData.functionName !== erc20Info.functionNameMap.decreaseAllowance) {
      return null
    }
    const [decodedByInputDataSpender, decodedByInputDataValue] = decodedByInputData.args
    ...
    for (const log of logs) {
      ...
      if ((event as any).eventName !== erc20Info.eventNameMap.Approval) continue
