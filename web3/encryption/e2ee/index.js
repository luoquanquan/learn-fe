...
function deriveSharedKey(privateKey, otherPublicKey) {
  ...
  // 取后 32 字节
  const sharedKey = crypto.createHash('sha256').update(sharedSecret.slice(1)).digest() // 32 bytes
...
console.log(`Current log: aliceSharedKey.toString("hex"): `, aliceSharedKey.toString('hex'))
console.log(`Current log: bobSharedKey.toString("hex"): `, bobSharedKey.toString('hex'))
// 两个人生成的共享密钥应该是一样的
console.log(`Current log: equal: `, aliceSharedKey.toString('hex') === bobSharedKey.toString('hex'))
